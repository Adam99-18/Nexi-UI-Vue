...
<template>
  <div>
    <AntdButton type="primary">新增数据</AntdButton>
  </div>
</template>
```
:::
...
<template>
  <div>
    <AntdButton type="primary" @click="modalFormRef.open()">
      新增用户
    </AntdButton>
    <AntdButton style="margin-left: 8px" @click="modalFormRef.open({ id: 1, name: '张三', role: 'admin', status: 1 })">
      编辑用户
    </AntdButton>
...
import AntdModalForm from '@/components/antd/modal-form.vue'
...
<template>
  <div>
    <AntdButton type="primary" @click="customRef.open()">
      自定义字段
    </AntdButton>
...
import { ref } from 'vue'
import AntdButton from '@/components/antd/button.vue'
import AntdSwitch from '@/components/antd/switch.vue'
import AntdModalForm from '@/components/antd/modal-form.vue'

const customRef = ref()

const customColumns = [
  { label: '名称', field: 'name', component: 'AntdInput', placeholder: '请输入' },
  { label: '启用状态', field: 'status', component: 'AntdSwitch', useSlot: true },
]
</script>
```
